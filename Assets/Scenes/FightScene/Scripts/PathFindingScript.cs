...
public class PathFindingScript : MonoBehaviour {
	...
	Grid grid;
	static PathFindingScript instance;
	...
	Vector2[] SimplifyPath(List<Node> path) {
		...
		for (int i = 1; i < path.Count; i ++) {
			Vector2 directionNew = new Vector2 (path[i-1].gridX, path[i-1].gridY);//path[i-1].gridX - path[i].gridX,path[i-1].gridY - path[i].gridY);
